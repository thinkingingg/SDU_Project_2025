...
1. **频域方法的优势与权衡**  
   相较于空域水印，频域嵌入利用了图像在频率空间的冗余特性，能够在保证较高不可见性的同时提高对常见信号处理攻击的鲁棒性。实验中，通过调整水印强度因子 $\alpha$ 和嵌入半径 $R$，可以明显观察到水印在“不可见性”与“鲁棒性”之间的平衡关系。

2. **掩模设计的重要性**  
   圆形掩模 $M(u,v)$ 决定了水印的嵌入位置和范围。实验表明，将水印嵌入在中低频区域既能减少视觉失真，又能有效抵抗压缩和亮度、对比度调整等操作。但对于几何攻击（旋转、裁剪），缺乏配套的同步恢复机制仍是一个弱点。

3. **鲁棒性差异的原因分析**  
   在鲁棒性测试中，几何变换（如旋转）和随机噪声会打乱频域能量分布，导致水印信息严重丢失；而对比度调整和缩放等线性或相对平稳的变换对频域幅度谱的破坏较小，因此提取性能保持较好。这说明算法对“全局平滑变化”更具容忍度。

4. **工程实现的收获**  
   通过编写 `DigitalWatermark` 类，将嵌入、提取和鲁棒性测试模块化，不仅方便了代码调试，也为后续扩展（如多种嵌入掩模、不同变换域）提供了良好的结构基础。同时，结合 PSNR 和 NCC 这类定量指标，让水印性能评估更加客观。

5. **改进方向**  
   本系统在频域水印的基本功能和部分鲁棒性方面表现良好，但对于几何攻击的抗性不足。后续可考虑引入几何校正（如基于特征点的配准）、冗余嵌入或切换到小波变换(DWT)等多分辨率方法，以进一步提升在实际复杂环境下的稳定性和安全性。
